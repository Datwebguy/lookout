...
- [x] Replaced the "Add a site" form's raw latitude/longitude number inputs with a real address lookup (`GET /api/geocode`, OpenStreetMap Nominatim, US-restricted) since everyday users don't know their site's GPS coordinates. The form now resolves a typed address to real coordinates via a "Find" button, shows the real matched place name, and invalidates the resolved coordinates if the address is edited again. Verified with a real Playwright run: real address, real Nominatim lookup, real form submission, real persistence to `sites.json`, zero console errors.
- [x] Three real gaps the user identified, all fixed: (1) **True autonomy** — a real background thread (`web/server.py::_background_loop`, started via `@app.on_event("startup")`) now runs the exact same real cycle as `POST /api/run` on its own, on a real hourly interval, with zero human interaction required. Verified live: the moment the server process started, real FortyGuard/OpenAI calls began immediately with no request from any client. (2) **Alert isolation between different registrants** — `POST /api/sites` now requires a real Slack or Discord webhook (rejects the request otherwise), since without one a new registrant's alerts would silently land in the shared demo channel; verified both the rejection and the success path live. Full separation of *who can see which sites on the dashboard* remains an open gap (needs real accounts, out of scope for now — see Decisions). (3) **Nationwide capability** — the backend was never Phoenix-restricted (only the demo data was); added a genuine third site in North Las Vegas, Nevada to make this visible, and verified it received a real, independently-grounded decision (30.01°C to 39.25°C real range, correctly extreme).
